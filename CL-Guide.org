...
|------------------------+----------------------------------------|
| :use                   | Inherits (uses) this packages for      |
|                        | this particular package. If you want   |
|                        | to use symbols of the given package,   |
|                        | those symbols must be exported there.  |
|------------------------+----------------------------------------|
| :export                | Exports the given symbols to let the   |
|                        | users of your package use this symbols |
|                        | without having to call the full path   |
|                        | (e.g. instead of                       |
|                        | (com.testsuite.app:save) just (save))  |
|------------------------+----------------------------------------|
| :import-from           | Imports a particular symbol from       |
|                        | another package to use this symbol     |
|                        | without having to use its full path.   |
|------------------------+----------------------------------------|
| :shadow                | Shadows a particular symbol from all   |
|                        | imports to avoid name conflicts. The   |
|                        | full path has to be used instead.      |
|------------------------+----------------------------------------|
| :shadowing-import-from | Imports a symbol and automatically     |
|                        | shadows all other symbols with the     |
|                        | same name to avoid conflicts.          |
|------------------------+----------------------------------------|
...
1. Every package is a file
   - The Packages' dependencies are decentralized
   - Enforces information hiding and loose coupling
   - Might cause troubles with not exported symbols
   - Current state of the art

2. One file defines all packages and the package functions are
   implemented in one or more files
   - The Packages' dependencies are centralized in only one file
   - Much easier symbol handling but abolishes loose coupling of the
     files
...
* ORM
** crane[fn:48]
crane is built on sqxl. That means that you have the power of an ORM
and you can always create your own queries.
...
** Weblocks
*** Concepts
In Weblocks widgets are used to create a HTML page. This means that
Weblocks creates the wrapper for you and you can place your own
widgets in it. The widgets work like trees. Starting point is the
function INIT-USER-SESSION. There the developer has to give the root
widget a value. This could be a LIST, a STRING or a WIDGET.
*** Widget
Widgets are specific classes for Weblocks. There is an own macro for
it called DEFWIDGET. Each widget has one very important method:
RENDER-PAGE-BODY. This method prints HTML code to an output
stream.
#+LATEX: \\
#+BEGIN_SRC lisp
(defwidget hello-world-widget ()
 ((text
   :initform (error "Must supply a text.")
   :initarg :text
   :reader text)))

(defmethod render-widget-body ((this hello-world-widget) &key)
  (with-html
    (:p
      ;; calling STR is needed by CL-WHO for foreign functions within
      ;; the WITH-HTML macro
      (str
        (text this)))))
#+END_SRC
